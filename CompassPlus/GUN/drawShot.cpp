...
void drawMain(int bulletX, int bulletY, int bulletR, int meetPlace,  int powerPercent, int gunX, int gunY, int gunAngle,int Nx, int Ny, int sign, int statusHits, int statusMisses,int targetX, int targetY, int BulletTime);
void drawBoom(int boomX, int boomY);
...
int main()
{
    ...
    drawMain(300, 300, 6, 100, 100,50, 405, 60, 60, 10, 0, 10, 25, 954, 628, 100);
...
void drawMain(int bulletX, int bulletY, int bulletR, int meetPlace,  int powerPercent, int gunX, int gunY, int gunAngle,int Nx, int Ny, int sign, int statusHits, int statusMisses,int targetX, int targetY, int BulletTime)
{
    ...
    while (i++  < gunAngle)
  {
    ...
    drawStatus(10, 25, 3);
    drawNomber0(Nx, Ny,1);
    drawTarget(954, 600 - i * 5 * sin(1.57 * i / 100));
    ...
    while (k++ < powerPercent)
  {
    ...
    drawStatus(10, 25, 3);
    drawNomber0(Nx, Ny,1);
    drawTarget(954, 600 - gunAngle * 5 * sin(1.57 * gunAngle / 100) - k * 2 * sin(1.57 * k / 100));
    ...
    while (v++ < BulletTime)
  {
    ...
    drawStatus(10, 25, 3);
    drawNomber1(Nx, Ny,1);
    drawTarget(954, 600 - gunAngle * 5 * sin(1.57 * gunAngle / 100) - powerPercent * 2 * sin(1.57 * powerPercent  / 100) - v * 5 * sin(1.57 * v / 20));
    txSetColor(TX_BLACK);
    txSetFillColor(TX_WHITE);
    txRectangle(POWER_X, POWER_Y, POWER_X + POWER_W, POWER_Y + POWER_H);
    txSetFillColor(TX_RED);
    ...
    drawBoom(gunX + 100 - (gunAngle / 3) * sin(3.14 * gunAngle / 60) + v * 0.2 * powerPercent - 130, gunY - gunAngle - v * 5 * sin(1.57 * gunAngle / 60) + v * v * v / 600 - 25);
...
void drawTarget(int targetX, int targetY)
{
    ...
    txCircle(targetX + TARGET_R / 2, targetY - TARGET_R / 2,  TARGET_R / 4);
    txCircle(targetX - TARGET_R / 2, targetY - TARGET_R / 2,  TARGET_R / 4);
