...
//!
//!          $Version: 0172a, Revision: 204 $
//!          $Copyright: (C) Ded (Ilya Dedinsky, http://ded32.net.ru) $
//!          $Date: 2012-06-20 16:09:42 +0400 $
//!
...
#if !defined (__TXLIB_H)                         // <<<<<<<<< The code is here, unfold it
#define       __TXLIB_H
...
#define _TX_VERSION           _TX_V_FROM_CVS ($versioninfo: , TXLib.h, 0172a, 204, 2012-06-20 16:09:42 +0400, "Ded (Ilya Dedinsky, http://ded32.net.ru)", $)
#define _TX_AUTHOR            _TX_A_FROM_CVS ($versioninfo: , TXLib.h, 0172a, 204, 2012-06-20 16:09:42 +0400, "Ded (Ilya Dedinsky, http://ded32.net.ru)", $)
...
#define _TX_VER               _TX_v_FROM_CVS ($versioninfo: , TXLib.h, 0172a, 204, 2012-06-20 16:09:42 +0400, "Ded (Ilya Dedinsky, http://ded32.net.ru)", $)
...
//!
//! @warning Если используется многофайловый проект (с раздельной компиляцией), то графические
//!          функции TXLib, вызванные из файла проекта, будут работать только с тем окном,
//!          которое создано <b>в этом же файле</b> проекта. Если проект состоит, скажем, из
//!          файлов @c main.cpp и @c game.cpp, и в файле @c main.cpp создается графическое окно,
//!          то функции из @c game.cpp не смогут рисовать в нем. (Однако @c game.cpp сможет
//!          создать свое собственное окно.) @n
//!          Если такой программе нужно одно окно, то в проект следует включить файл, ответственный
//!          за рисование, скажем, @c graphics.cpp, и выводить графику только через функции этого
//!          файла. Такой файл (или библиотеку) в больших проектах часто называют графическим
//!          движком.
//!
//! @see     txOK()
//! @usage @code
...
//!
//! @param   x0          X-координата верхнего левого  угла прямоугольника,
//!                        описанного вокруг эллипса, содержащего сектор (см. txEllipse)
//! @param   y0          Y-координата верхнего левого  угла прямоугольника
//! @param   x1          X-координата нижнего  правого угла прямоугольника
//! @param   y1          Y-координата нижнего  правого угла прямоугольника
...
//!
//! @param   x0          X-координата верхнего левого угла прямоугольника,
//!                        описанного вокруг эллипса, содержащего хорду (см. txEllipse)
//! @param   y0          Y-координата верхнего левого угла прямоугольника
//! @param   x1          X-координата нижнего правого угла прямоугольника
//! @param   y1          Y-координата нижнего правого угла прямоугольника
...
//!          тогда значения каналов R,G,B для этого пикселя также станут 0 (это черный цвет).
//!        - Если значение альфа-канала для некоторого пикселя равно 255 (полная непрозрачность),
//!          тогда значения каналов R,G,B для этого пикселя не изменятся.
...
//!
//!          В возвращаемом значении выставленный в единицу 1-й (младший) бит означает нажатую
//!          левую Кнопку Мыши, 2-й - правую, 3-й - среднюю. @n
//!          Например, возвращенное число 5 (двоичное 101) означает одновременное нажатие
//!          средней и левой Кнопок, но не правой Кнопки.
//!
//! @see     txMouseX(), txMouseY(), txMousePos(), txMouseButtons()
//! @usage @code
...
//!          получается размер массива в элементах. Он <b>не проверяет,</b> можно ли его правильно
//!          вычислить, и при неправильном использовании может выдать <b>неверный размер.</b>
//!
...
//!          int Maru = ROUND (weight); // We should use ROUND() because Maru is so round.
//! @endcode
//! @hideinitializer
//}-------------------------------------------------------------------------------------------
...
//!
//!          Перед вызовом любых функций Win32 GDI необходимо заблокировать холст функцией
//!          txLock() и затем разблокировать с помощью txUnlock(). Это связано с тем, что
//!          обновление содержимого окна (для тех, кто поймет - обработка сообщения WM_PAINT)
//!          в библиотеке TXlib происходит в отдельном вспомогательном потоке. Надолго блокировать
//!          его нельзя - при заблокированном потоке окно не обновляется.
//!
...
//!          высокоуровневого механизма txBegin() / txEnd() / txUpdateWindow(), который не
...
//!          то команда GDI не выполняется, а txGDI() возвращает значение false. @n
//!
...
//!          клавишу два раза. @n
//!
//! @note    Если ранняя инициализация запрещена, но создается окно TXLib с помощью
//!          txCreateWindow(), то библиотека все равно будет инициализирована.
//!
//!          Может задаваться перед @b включением TXLib.h в программу.
//!
//! @see     txCreateWindow(), _TX_ALLOW_KILL_PARENTS, _TX_WAITABLE_PARENTS, _txConsoleMode
//! @usage @code
//!          #define _TX_NOINIT
//!          #include "TXLib.h"
//! @endcode
...
//!          не заблочила секцию, деструктор ее не разблочивает. Есть оператор для проверки,
//!          заблочилась ли секция или нет (см. конструкторы класса и оператор @c bool).
//!
...
//!           - Остальные элементы описывают контролы (порядок задания параметров контрола
//!             похож на порядок параметров в ресурсном скрипте)
...
const char* txInputBox (const char* text, const char* caption, const char* input)
    ...
    //     диалога          | элемента  | тор,связан- |-----------------| (см. описание элементов
    //                      |           | ный с ним   | X | Y |Шир.|Выс.|  диалога в MSDN)
    //----------------------+-----------+-------------+---+---+----+----+---------------------
    //                      |           |             |   |   |    |    |
...
#define __TX_EVENT_QUEUE__
#define FLORA_OS_WINDOWS
#if defined (__TX_EVENT_QUEUE__)
#ifndef TXLIBDEF_H
#define	TXLIBDEF_H

namespace flora {

   template <class T> void construct(T& x)
   { new(&x) T; }

   template <class T> void reconstruct(T& x)
   { x.~T(); new(&x) T(); }
...
template <class T> class T_Queue {
public:
    T_Queue(int Size,short Delta=0);
    T_Queue(const T_Queue<T>& q);
    ~T_Queue();

    T_Queue<T>& operator = (const T_Queue<T>& q);
                operator void* ()            { return empty() ? 0 : this; }
    bool        operator ! () const          { return empty(); }
    T&          operator [](int index)       { return data[index]; }
    const T&    operator [](int index) const { return data[index]; }
    T_Queue<T>& operator << (const T& item)  { putTail(item); return *this; }
    T_Queue<T>& operator >> (T& item)        { getHead(item); return *this;}

    bool empty()  const        { return tail_==prev(head_); }
    bool full()   const        { return next(tail_)==prev(head_); }
    int  filled() const        { return empty()?0 : tail_-head_+1 + (tail_>=head_ ? 0 : highBound+1); }
    int  size()   const        { return highBound+1; }
    void flush();
    int  indexOfTail()   const { return tail_; }
    int  indexOfHead()   const { return head_; }
    int  prev(int index) const { return (index<=0 ? highBound : index-1); }
    int  next(int index) const { return (index>=highBound ? 0 : index+1); }

    bool putTail(const T& item);
    bool putHead(const T& item);
    bool getTail(T& item);
    bool getHead(T& item);
    bool putTail();
    bool putHead();
    bool getTail();
    bool getHead();
    bool put(const T& item)       { return putTail(item); }
    bool get(T& item)             { return getHead(item); }
    bool put()                    { return putTail(); }
    bool get()                    { return getHead(); }

    T&   tail()                   { return data[tail_]; }
    T&   head()                   { return data[head_]; }
    const T& tail() const         { return data[tail_]; }
    const T& head() const         { return data[head_]; }

protected:
    int   tail_,head_,highBound;
    short delta;
    T*    data;

    void zero(int index)          { flora::reconstruct(data[index]); }
    bool expand();
    void copyItems(const T_Queue<T>& q);
};

template <class T> T_Queue<T>::T_Queue(int Size,short Delta)
{
    head_=0; tail_=highBound=Size; delta=Delta;
    data = NULL;
    if (Size) data = new T[size()];
}

template <class T> T_Queue<T>::T_Queue(const T_Queue<T>& q)
{
    head_=0; tail_=highBound=q.highBound; delta=q.delta;
    data = new T[size()];

	copyItems(q);
}

template <class T> T_Queue<T>::~T_Queue()
{
    if (data) delete[] data;
}

template <class T> T_Queue<T>& T_Queue<T>::operator = (const T_Queue<T>& q)
{
	flush();
	copyItems(q);
	return *this;
...
template <class T> void T_Queue<T>::copyItems(const T_Queue<T>& q)
{
	if (!q.empty())
	{
		int i = q.indexOfHead();
		while (i != q.next(q.indexOfTail()))
		{
			put(q[i]);
			i = q.next(i);
		}
...
template <class T> void T_Queue<T>::flush()
{ while (get()); }

template <class T> bool T_Queue<T>::getTail(T& item)
{
...
template <class T> bool T_Queue<T>::getHead(T& item)
{
   if (empty()) return false;
   item=data[head_];
   zero(head_);
...
template <class T> bool T_Queue<T>::putTail(const T& item)
{
   if (full() && !expand()) return false;
   data[tail_=next(tail_)]=item;
   return true;
}

template <class T> bool T_Queue<T>::putHead(const T& item)
{
   if (full() && !expand()) return false;
   data[head_=prev(head_)]=item;
   return true;
}

template <class T> bool T_Queue<T>::getTail()
{
   if (empty()) return false;
   zero(tail_);
   tail_=prev(tail_);
...
template <class T> bool T_Queue<T>::getHead()
{
   if (empty()) return false;
   zero(head_);
   head_=next(head_);
...
template <class T> bool T_Queue<T>::putTail()
{
   if (full() && !expand()) return false;
   tail_=next(tail_);
   return true;
...
template <class T> bool T_Queue<T>::putHead()
{
   if (full() && !expand()) return false;
   head_=prev(head_);
   return true;
...
template <class T> bool T_Queue<T>::expand()
{
   if (delta==0) return false;
   int h=highBound + (delta>0?delta:flora::max(1,highBound*int(-delta)/100));
   T *d = new T[h+1];
   if (empty())
      tail_=h;
   else if (tail_>=head_) {
      T *p1=d,*p2=data+head_;
      for (int i=tail_-head_+1;i>0;i--)
         *p1++ = *p2++;
      tail_-=head_; head_=0;
      }
   else {
      T *p1=d,*p2=data+head_;
      int i;
      for (i=highBound-head_+1;i>0;i--)
         *p1++ = *p2++;
      p1=d+highBound-head_+1,p2=data;
      for (i=tail_+1;i>0;i--)
         *p1++ = *p2++;
      tail_=tail_-head_+highBound+1; head_=0;
      }
   highBound=h;
   delete[] data;
   data=d;
...
//---------------------------- TThreadEvent --------------------------------------
struct TEvent;
class AKIMPEXP TThreadEvent {
public:
   TThreadEvent();
   ~TThreadEvent();
   void pulse();
   void set();
   bool wait(dword ms);    // true - signaled, false - timeout or error
   void waitInfinite();
   void* getEvent();
protected:
   TEvent *event;
...
TThreadEvent::TThreadEvent()
{
   event=new TEvent();
#if defined(FLORA_OS_WINDOWS)
   event->event=CreateEvent(0,false,false,0);
#else
   event->setted=false;
...
void TThreadEvent::pulse()
{
#if defined(FLORA_OS_WINDOWS)
   PulseEvent(event->event);
#else
...
bool TThreadEvent::wait(dword ms)  // true - signaled, false - timeout or error
{
#if defined(FLORA_OS_WINDOWS)
   return WaitForSingleObject(event->event,ms) == WAIT_OBJECT_0;
#else
   bool result = false;
   if (ms == 0) {
      pthread_mutex_lock(&event->mutex);
      ...
      timespec tm;
      tm.tv_nsec = (tv.tv_usec + (ms % 1000) * 1000) * 1000;
      tm.tv_sec = tv.tv_sec + (ms / 1000) + (tm.tv_nsec / 1000000000);
      tm.tv_nsec %= 1000000000;

      pthread_mutex_lock(&event->mutex);
      result = event->setted || (pthread_cond_timedwait(&event->event,&event->mutex,&tm) == 0);
      event->setted=false;
      pthread_mutex_unlock(&event->mutex);
   }
   return result;
...
void TThreadEvent::waitInfinite()
{
#if defined(FLORA_OS_WINDOWS)
...
struct TxEvent {
	enum Type { TX_NONE, TX_MOUSE_MOVED, TX_MOUSE_PRESS, TX_MOUSE_RELEASE, TX_KEY_PRESS, TX_CHAR, TX_KEY_RELEASE };
	TxEvent() : type(TX_NONE) {}
	TxEvent(TxEvent::Type t, const POINT &p, int c) : type(t), code(c), pos(p) {}
	TxEvent(TxEvent::Type t, int c) : type(t), code(c), pos() {}
	TxEvent(const TxEvent &e) : type(e.type), code(e.code), pos(e.pos) {}
	Type  type;
	POINT pos;
	int   code;
};

class T_TxEventQueue : public flora::T_Queue<TxEvent> {
public:
	T_TxEventQueue(int size) : flora::T_Queue<TxEvent>(size) {}
	bool putEvent(const TxEvent &event);
	bool getEvent(TxEvent &event, int ms);
	flora::TThreadEvent thread_event;
} tx_event_queue(100);

bool T_TxEventQueue::putEvent(const TxEvent &event)
{
	bool ret;
   txLock();
   if ((event.type == TxEvent::TX_MOUSE_MOVED) && !empty() && (tail().type == event.type))
   {
		tail().pos = event.pos; tail().code = event.code;
	}
	else
	{
		if (empty()) thread_event.set();
		ret = put(event);
	}
   txUnlock();
   return ret;
...
bool T_TxEventQueue::getEvent(TxEvent &event, int ms)
{
	bool ret = false;
	do
	{
	    ...
	    if (!ret)
	    {
		    if (ms < 0)
		    {
				thread_event.waitInfinite();
			}
			else
			{
				if (!thread_event.wait(ms)) return false;
			}
		}
	} while (!ret);
    return ret;
...
TxEvent getNextEvent(int ms = -1)
{
	TxEvent event;

    bool ret = tx_event_queue.getEvent(event, ms);
    if (!ret) event.type = TxEvent::TX_NONE;

    return event;
}

#endif	/* TXLIBDEF_H */
#endif  /* __TX_EVENT_QUEUE__ */
...
bool             _txCanvas_OnCHAR       (HWND wnd, WPARAM ch, LPARAM key);
...
//
//           Если вы пишете свою библиотеку и используете TXLib.h как примеру, не следуйте ему
//           не делайте так же. Здесь это сделано только в образовательных целях.
//
...
void _txCleanup()
    {
$1  Sleep (_txWindowUpdateInterval);
...
$   if (canvas && waitableParent && _txNOP (_TX_ALLOW_KILL_PARENT))
...
$           return SendMessageTimeoutW (window, WM_GETTEXT, (WPARAM) size, (LPARAM) text,
                                        SMTO_BLOCK | SMTO_ABORTIFHUNG, _TX_TIMEOUT, NULL);
            }
...
$   bool isMaster = !!(GetWindowLong (txWindow(), GWL_STYLE) & WS_SYSMENU);
    ...
    #ifndef NDEBUG
    OutputDebugString (_TX_VERSION " - STARTED\n");
    #endif
...
$   DestroyIcon (icon32);        // calls will possibly fail, and we'll get resource leak.
...
$       exit ((int) msg.wParam);
    ...
    #ifndef NDEBUG
    OutputDebugString (_TX_VERSION " - STOPPED\n");
    #endif
...
#if defined (__TX_EVENT_QUEUE__)
void _txCanvas_EventProc (HWND wnd, UINT msg, WPARAM wpar, LPARAM lpar);
#endif
...
#if defined (__TX_EVENT_QUEUE__)
    _txCanvas_EventProc (wnd, msg, wpar, lpar);
#endif
    return DefWindowProc (wnd, msg, wpar, lpar);
...
bool _txCanvas_OnCHAR (HWND, WPARAM ch, LPARAM key)
    {
...
$   evt.Event.KeyEvent.wVirtualScanCode  = (unsigned char) (key >> 16);
$   evt.Event.KeyEvent.wVirtualKeyCode   = (WORD) MapVirtualKey (evt.Event.KeyEvent.wVirtualKeyCode, 3);
...
#if defined (__TX_EVENT_QUEUE__)
void _txCanvas_EventProc (HWND , UINT msg, WPARAM wpar, LPARAM )
    {

    switch (msg)
        {
        case WM_CHAR:
            tx_event_queue.putEvent(TxEvent(TxEvent::TX_CHAR, (unsigned char) wpar));
            $ break;
        case WM_KEYDOWN:
            tx_event_queue.putEvent(TxEvent(TxEvent::TX_KEY_PRESS , (unsigned char) wpar));
            $ break;
        case WM_KEYUP:
            tx_event_queue.putEvent(TxEvent(TxEvent::TX_KEY_RELEASE, (unsigned char) wpar));
            $ break;

        case WM_LBUTTONUP:
        case WM_RBUTTONUP:
        case WM_MBUTTONUP:
	        tx_event_queue.putEvent(TxEvent(TxEvent::TX_MOUSE_RELEASE  , _txMousePos, (int)wpar));
            $ break;
        case WM_LBUTTONDOWN:
        case WM_RBUTTONDOWN:
        case WM_MBUTTONDOWN:
	        tx_event_queue.putEvent(TxEvent(TxEvent::TX_MOUSE_PRESS, _txMousePos, (int)wpar));
            $ break;
        case WM_MOUSEMOVE:
	        tx_event_queue.putEvent(TxEvent(TxEvent::TX_MOUSE_MOVED   , _txMousePos, (int)wpar));
            $ break;
        default:
            $ break;
        }
...
$1  //{ Overriding the missing names, if the set is uncomplete
        ...
        #define __DESCRIPTION  "(Да, мне лень задать описание)." " \t\t" "#define __DESCRIPTION to override project role."
        #endif
...
$   return txGDI (!!(Win32::BitBlt (dest, ROUND (xDest), ROUND (yDest),
...
$   return (Win32::TransparentBlt)?
        txGDI (!!(Win32::TransparentBlt (dest, ROUND (xDest), ROUND (yDest),
                                               ROUND (width), ROUND (height),
                                         src,  ROUND (xSrc),  ROUND (ySrc),
                                               ROUND (width), ROUND (height), transColor)))
    :
        txGDI (!!(Win32::BitBlt         (dest, ROUND (xDest), ROUND (yDest),
                                               ROUND (width), ROUND (height),
...
$   return (Win32::AlphaBlend)?
        txGDI (!!(Win32::AlphaBlend (dest, ROUND (xDest), ROUND (yDest),
                                           ROUND (width), ROUND (height),
                                     src,  ROUND (xSrc),  ROUND (ySrc),
                                           ROUND (width), ROUND (height), blend)))
    :
        txGDI (!!(Win32::BitBlt     (dest, ROUND (xDest), ROUND (yDest ),
                                           ROUND (width), ROUND (height),
                                     src,  ROUND (xSrc),  ROUND (ySrc), SRCCOPY))), false;
