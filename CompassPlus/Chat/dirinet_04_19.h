...
enum NetCommandType {NONE, CLIENT_MSG, SERVER_MSG, REG};
...
NetCommand readNextCommand(NetBuffer* netBuffer)
{
    ...
    if (s == '1')
    {
        ...
        if (readInt(netBuffer, 1, &hz) && readInt(netBuffer, 5, &dur)) {
            result.type = CLIENT_MSG;
            //result.name = netBuffer->data;
            netBuffer->consume(9);
            return result;
        }
    } else if (s == '2')
...
{
    if (command.type == CLIENT_MSG)
    {
        dest[0] = '1';
        int nameLen = putInt(command.nameLen, dest + 1);
        return nameLen;
    }
    else if (command.type == SERVER_MSG)
    {
        dest[0] = '2';
        return 1;
    }
