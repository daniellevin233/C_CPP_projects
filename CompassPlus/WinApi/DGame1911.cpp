...
void Game::updateTowersFocus()
{
    for (int i = 0; i < curTowerCount; i++)
    {
        ...
        } else
        {
            ...
            {
                POINT uPoint;
                uPoint.x = units[towers[i].lastFocusedUnit].x;
                uPoint.y = units[towers[i].lastFocusedUnit].y;
                POINT tPoint;
                tPoint.x = towers[i].x;
                tPoint.y = towers[i].y;
                if (dest(uPoint, tPoint) > getDamageR(towers[i]))
                {
                    needNewFocus = true;
                }
            }
...
int Game::focusTower(Tower t)
{
    ...
    for (int i = 0; i < curUnitCount; i++)
    {
        if (units[i].active)
        {
            ...
            uPoint.x = units[i].x;
            uPoint.y = units[i].y;
            double dist = dest(tPoint, uPoint);
            if (dist < minDist )
            {
                index = i;
                minDist = dist;
