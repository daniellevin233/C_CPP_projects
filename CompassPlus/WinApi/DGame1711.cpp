...
const double REDRAW_AFTER_TICKS = 5;
...
void Game::init()
{
    ...
    gunTowerPen = CreatePen(PS_SOLID, 6, RGB(0, 255, 0));
...
int Game::processMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    } else if (msg == WM_LBUTTONUP)
    {
        if (curControlState == NEW_TOWER)
        {
            ...
            towers[curTowerCount].lastFocusedUnit = 0;
            curTowerCount++;
            curControlState = NONE;
...
void Game::drawTower(HDC hdc, Tower tower)
{
    if (tower.type == TW_GUN)
    {
        ...
        double gx = 0;
        double gy = 0;
        printf("l %d u %d\n", l, tower.lastFocusedUnit);
        if (tower.lastFocusedUnit >= 0 && units[tower.lastFocusedUnit].active)
        {
            POINT towerP;
            towerP.x = tower.x;
            towerP.y = tower.y;
            POINT unitP;
            unitP.x = units[tower.lastFocusedUnit].x;
            unitP.y = units[tower.lastFocusedUnit].y;
