...
void Game::updateNewTower()
{
    ...
    allowedRect.bottom = WINDOW_H - TOP_GAME_BLOCK_H - TOWER_R;

    if (PtInRect(&allowedRect, p))
    {
        for (int i = 0; i < curTowerCount; i++)
        {

            if (dest(p, getTowerPoint(towers[i])) >= TOWER_R + 1)
            {
...
void Game::drawTowerButtons(HDC hdc)
{
    for (int i = 0; i < level.towerTypesCount; i++)
    {
        ...
        t.y = WINDOW_H - BOTTOM_GAME_BLOCK_H / 2;
...
POINT Game::getTowerPoint(Tower t)
{
    POINT p;
    p.x = t.x;
    p.y = t.y;
